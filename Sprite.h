...
namespace TRXEngine 
{
	...
	public:
		...
		void init(const std::string& path_to_texture, glm::vec4 pos_and_dim = glm::vec4(0), glm::vec4 uv_rec = {0.0f,0.0f,1.0f,1.0f}, float depth = 1.0f, TRXEngine::Color color = {255,255,255,255});
		...
		void setDimension(glm::vec2 new_dim) { m_pos_and_dim = glm::vec4(m_pos_and_dim.x,m_pos_and_dim.y,new_dim); }
		void setColor(TRXEngine::Color color) { m_color = color; }
